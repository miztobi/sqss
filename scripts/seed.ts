...
const fourChoiceQuestions1Kyu = [
	{
		questionId: 'q4_2026_houki_01',
		subject: '法規',
		chapter: '用途地域・用途制限',
		questionText: '第一種低層住居専用地域内において建築することができる建築物はどれか。',
		choices: [
			'1. 延べ面積が200㎡で、地上2階建ての郵便局（その用途に供する部分の床面積が150㎡）',
			'2. 延べ面積が300㎡で、地上2階建ての老人福祉センター',
			'3. 延べ面積が500㎡で、地上3階建ての中学校',
			'4. 延べ面積が600㎡で、地上2階建ての図書館'
		],
		correctAnswer: 2,
		explanation: '建築基準法別表第2(い)項に基づき、中学校および小学校・高校・老人ホーム・公衆便所・診療所等は第一種低層住居専用地域内に建築することができます。郵便局は床面積制限（500㎡以下かつ所定用途制限）に反する場合があり、図書館や老人福祉センターには床面積や用途の厳格な制限があります（特に中学校は無条件で建築可能です）。'
	},
	{
		questionId: 'q4_2026_keikaku_01',
		subject: '計画',
		chapter: '都市計画・近隣住区',
		questionText: 'ラドバーン方式（Radburn System）における歩車分離計画に関する記述として、最も不適当なものはどれか。',
		choices: [
			'1. 歩行者専用道路網を住区全体に張り巡らせ、住宅の玄関側（歩行者側）とアプローチさせる。',
			'2. 自動車用道路は袋小路（クルドサック）形式とし、住戸のサービスヤード（勝手口側）へ接続する。',
			'3. 住区の外周には幹線道路を配し、住区内を通過する自動車交通を排除する。',
			'4. 歩行者道路と自動車道路の交点には、平面交差の信号機を多く配置して安全を確保する。'
		],
		correctAnswer: 3,
		explanation: 'ラドバーン方式は完全な歩車分離を目指した計画であり、歩行者道路と自動車道路の交差部分は「立体交差（オーバーパスやアンダーパス）」とし、歩行者が自動車と一度も交差せずに住区内を安全に移動できるように設計されているため、平面交差を多用するのは誤りです。'
	},
	{
		questionId: 'q4_2026_kouzou_01',
		subject: '構造',
		chapter: '基礎構造・地盤',
		questionText: '杭基礎の設計に関する記述として、最も不適当なものはどれか。',
		choices: [
			'1. 支持杭の極限支持力は、杭頭付近の土の抵抗が最大になるため、地盤のN値のみから精度よく算定できる。',
			'2. 摩擦杭は、杭周面と周囲の土との間の摩擦力によって主として荷重を支持する杭である。',
			'3. 地震時における杭頭モーメントに対して、杭頭部と基礎スラブの接合部は適切な補強を行う必要がある。',
			'4. 同一建物において、異なる支持構造（直接基礎と杭基礎など）を混用することは不同沈下防止のため避けるべきである。'
		],
		correctAnswer: 0,
		explanation: '支持杭の先端支持力は「杭先端の地盤状況（支持層のN値）」に大きく依存します。杭頭（地表面付近）の土の抵抗は支持杭全体の支持力算定において支配的ではなく、極限先端支持力および杭周面摩擦力をベースに計算します。'
	},
	{
		questionId: 'q4_2026_seikou_01',
		subject: '施工',
		chapter: '鉄筋コンクリート工事',
		questionText: 'コンクリートの養生に関する記述として、最も不適当なものはどれか。',
		choices: [
			'1. コンクリートの打込み直後は、日光の直射や風による急激な乾燥を防ぐため、散水やシート養生を行う。',
			'2. 普通ポルトランドセメントを用いた場合、湿潤養生の期間は、日平均気温が15℃以上であれば、一般に5日間以上とする。',
			'3. 早強ポルトランドセメントを用いた場合の湿潤養生期間は、普通ポルトランドセメントの場合よりも長くする必要がある。',
			'4. 暑中コンクリート工事においては、水分蒸発速度が速いため、湿潤養生開始を早める必要がある。'
		],
		correctAnswer: 2,
		explanation: '早強ポルトランドセメントは化学反応（水和反応）が速く、数日で普通ポルトランドセメントと同等の強度を発現するため、湿潤養生期間は普通セメントの5日間に対して3日間と短く設定できます。'
	},
	{
		questionId: 'q4_2026_kankyo_01',
		subject: '環境_設備',
		chapter: '日射・日よけ設計',
		questionText: '建築物の日射遮蔽計画に関する記述として、最も不適当なものはどれか。',
		choices: [
			'1. 南向き of 窓に対しては、夏季の高度の高い日射を遮るために水平ひさし（庇）を設けることが有効である。',
			'2. 西向き of 窓に対しては、午後からの高度の低い西日を遮るために垂直可動ルーバーを設けることが有効である。',
			'3. 熱線反射ガラスやLow-E複層ガラス（日射遮蔽型）は、開口部からの日射熱流入を低減させる効果がある。',
			'4. 日射を遮るためのブラインドやロールスクリーンは、室内側に設置する方が室外側に設置する方よりも日射遮蔽効果が高い。'
		],
		correctAnswer: 3,
		explanation: '日よけ（ブラインドやすだれ等）は、室外側に設置する方が日射遮蔽効果が高くなります。室内側に設置すると、ブラインド自体が温まり室内の暖房器具のようになってしまうためです。'
	}
];
...
async function seed() {
	...
	for (const goal of goalsData) {
		...
		if (goal.goalId === '1kyu_kenchikushi') {
			...
			console.log('Writing 1kyu 4-choice questions...');
			for (const q4 of fourChoiceQuestions1Kyu) {
				await db
					.collection('goals')
					.doc(goal.goalId)
					.collection('four_choice_questions')
					.doc(q4.questionId)
					.set(q4);
			}
		}
